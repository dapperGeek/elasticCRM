...
$message = "Dear engineer $engineerName,  \n<br> Please be informed that a service call has been assigned to you. You are to visit $accountName custormer for service. \n<br> Kindly find details in the service ticket. \n<br> \n<br> Please do not reply to this email, this address is not monitored. Please Contact customer care.";
 ...
 // note the above parameters now referenced in the 'subject', 'html', and 'text' sections
    ...
    $json_string = array(
      ...
      'to' => array($engineerEmail,'talal@tenaui.com','SRIDHAR@tenaui.com','elfarra@tenaui.com','sameh@tenaui.com'
    ),
  ...
  function SellProductStock($supplier,$invoiceNo,$fileRef,$storeID,$product,$qty,$save,$invoiceDate,$sold){
        if(count($product) > 0){

                        $this->SellNewStockToProduct($supplier,$invoiceNo,$fileRef,$storeID,$product,$qty,$save,$invoiceDate,$sold);
  ...
  function addGoodsRecievedGoods($mainID,$productID,$currentQty,$addedQty){
    $sql = "INSERT INTO  goods_recieved_goods (goodRecievedID,productID,currentQty,AddedQty)VALUES (?,?,?,?)";
    $handle = $this->db->prepare($sql);
  ...
  public function getAllGoodsAddedForRecieve($id){
   ...
   public function SellNewStockToProduct($supplier,$invoiceNo,$fileRef,$storeID,$product,$qty,$save,$invoiceDate,$sold){
        $sold_ = "SOLD";
        if($sold == 1){$sold_="SOLD";}else if($sold == 2){$sold_ == "TRANSFERED";}
        ...
        $count = count($product);
            if ($count >0 && !empty($product)) {
                    ...
                    $this->showMsg('Success', 'This transaction has been made on this ticket <a href="'.$this->host.'goods-sold-ticket/'.$mainID.'"class="btn btn-success">CLICK HERE TO OPEN TICKET</a>',2);
                     //$accountName = $this->getSingleAccountInformation($accountID)['Name'];
                     //$machineName = $this->getSingleMachineInformation($machineID)['machine_code'];
                     //$message = "added a new purchase for a machine : ".$machineName. " assigned to ".$accountName;
                     //$this->createActivityNotifications($message);
...
$message = "Dear Customer Care, \n<br> Please be informed that a service call has been followed up by engineer $engineerName for $accountName custormer ticket No $ticketNo. \n<br> Kindly find details in the followed up call. \n<br> \n<br> Please do not reply to this email, this address is not monitored. Please Contact customer care.";
 ...
 // note the above parameters now referenced in the 'subject', 'html', and 'text' sections
    ...
    $json_string = array(
      ...
      'to' => array('igweuu@tenaui.com','talal@tenaui.com','SRIDHAR@tenaui.com','elfarra@tenaui.com'),
      'category' => 'test_category'
    );
 ...
 public function getStaffDesignation(){
  ...
  public function changeUserPass($user_currentPass,$old_pass,$new_pass,$confirm_pass,$username)
  {
   ...
   elseif($new_pass==$confirm_pass){
    ...
    public function UpdateCreditNote($nQty,$invoiceNo,$GetId)
    {
       $update_UpdateCreditNote = $this->db->query("UPDATE goodslog SET returned= returned +$nQty ,remain= remain +$nQty WHERE InvoiceNo='$invoiceNo' AND productID='$GetId'");
...
$sql = "INSERT INTO creditnote
 ...
 public function getAllReturnTicket(){
      $sql = "SELECT creditnote.productId, goodslog.productID, creditnote.id,creditnote.productName,goodslog.returned,creditnote.Code,creditnote.invoiceNo,creditnote.custormerName,creditnote.invoiceDate,creditnote.returnDate,creditnote.store,creditnote.unit,creditnote.ticketGen FROM creditnote INNER JOIN goodslog ON creditnote.productId=goodslog.productID WHERE goodslog.returned>0";
        $myArray = array();
      $handle = $this->db->prepare($sql);
      //$handle->bindValue(1,$id);
      $handle->execute();
 ...
 public function getAllReturnTicketId($id){
      $sql = "SELECT creditnote.productId, goodslog.productID, creditnote.id,creditnote.productName,goodslog.returned,creditnote.Code,creditnote.invoiceNo,creditnote.custormerName,creditnote.invoiceDate,creditnote.returnDate,creditnote.store,creditnote.unit,creditnote.ticketGen FROM creditnote INNER JOIN goodslog ON creditnote.productId=goodslog.productID WHERE creditnote.id=$id AND goodslog.returned>0";
        $myArray = array();
      $handle = $this->db->prepare($sql);
      //$handle->bindValue(1,$id);
      $handle->execute();
